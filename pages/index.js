...
const router = useRouter();
  return (
    <>
      ...
      <Portal>
      <div className="flex flex-col items-center justify-center fixed z-40 right-0 h-32 w-7 top-1/3 transform -translate-y-1/2 bg-black text-white md:top-1/2 md:h-44 md:w-10">
        <button
        onClick={() => router.push("/projects")}
         className="-rotate-90 h-7  w-32 border-1.5 border-primary1 text-primary1 text-sm text-center md:text-base md:h-44 md:w-10">
          View Projects_
        </button>
      </div>
      </Portal>
